...
class CaptureManager(object):
    ...
    def exitFrame(self):
        ''' Draw to the window . write to fiels . release the frame '''

        #check whether any grabbed frame is retrievable
        #the getter may retrieve and cache the frame
        if self._framesElapsed is None:
            self._enteredFrame = False
            return

        #update the fps estimste snd related variables.
        ...
        if self._videoWriter is None:
            ...
            if fps <= 0.0 :
                ...
                if self._framesElapsed < 20 :
                    # wait until more frames elapes so that the 
